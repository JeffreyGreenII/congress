...
def fetch_senate_committee_meetings(committees, options):
    ...
    for node in dom.xpath("meeting"):
        ...
        if options.get("debug", False):
            print(
                "[senate][%s][%s] Found meeting in room %s at %s."
                % (committee_code, subcommittee_code, room, occurs_at.isoformat())
            )
...
def fetch_house_committee_meetings(committees, options):
    ...
    committee_html = utils.download(
        ...
        "committee_schedule/house_overview.html",
        opts,
    )
...
def load_xml_from_page(
    ...
    # Parse the XML.
    try:
        meeting = parse_house_committee_meeting(
            event_id,
            dom,
            existing_meetings,
            committees,
...
def parse_house_committee_meeting(
    ...
    try:
        ...
        occurs_at = (
            dom.xpath("string(meeting-details/meeting-date/calendar-date)")
            + " "
            + dom.xpath("string(meeting-details/meeting-date/start-time)")
        )
    ...
    for committee_code, subcommittee_code in orgs:
        ...
        for mtg in existing_meetings:
            if (
                mtg["house_event_id"] == int(event_id)
                and mtg.get("committee", None) == committee_code
                and mtg.get("subcommittee", None) == subcommittee_code
            ):
                guid = mtg["guid"]
                break
        ...
        if options.get("debug", False):
            print(
                "[house][%s][%s] Found meeting in room %s at %s"
                % (committee_code, subcommittee_code, room, occurs_at.isoformat())
            )
...
def text_from_pdf(pdf_path):
    ...
    try:
        subprocess.check_call(
            'pdftotext -layout "%s" "%s"' % (real_pdf_path, real_text_path), shell=True
        )
