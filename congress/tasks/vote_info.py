...
def fetch_vote(vote_id, options):
    ...
    if vote_chamber == "h":
        url = "http://clerk.house.gov/evs/%s/roll%03d.xml" % (
            vote_session_year,
            int(vote_number),
        )
    else:
...
def parse_house_vote(dom, vote):
    ...
    all_voters.sort(key=lambda v: len(v["display_name"]), reverse=True)
    for v in all_voters:
...
def get_vote_category(vote_question):
    ...
    mapping = (
        ...
        # $1 is a name like "Broun of Georgia"
        (r"^On .*Motion ", "procedural"),
